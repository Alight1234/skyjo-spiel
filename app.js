    ...
    return new Promise((resolve) => {
        unsubscribeGameListener = onSnapshot(gameDocRef, (docSnap) => {
            if (docSnap.exists()) {
                ...
                if (currentGameId === gameId) { // Nur resetten, wenn es das *aktuelle* Spiel war
...
async function leaveGame() {
    ...
    const gameIdToLeave = currentGameId; // Spiel-ID merken
    currentGameId = null; // Aktuelles Spiel sofort auf null setzen

    // Listener und Timeout sofort stoppen
    if (unsubscribeGameListener) {
    ...
    try {
        const gameDocRef = doc(db, getGameCollectionPath(), gameIdToLeave); // Gemerkte ID verwenden
        const gameDocSnap = await getDoc(gameDocRef);
        if (gameDocSnap.exists()) {
            ...
            if (humanPlayers.length === 1 && playerToRemove && !playerToRemove.isAI) { // Letzter menschlicher Spieler geht
                ...
                if (playerToRemove.isHost && game.hostId === userId) { // Host geht
                    const newHost = humanPlayers.find(p => p.id !== userId); // Finde einen anderen Menschen
                    if (newHost) {
                        console.log("Host verlässt, neuer Host:", newHost.id);
                        newHost.isHost = true;
                        // Spieler entfernen UND neuen Host setzen
                    ...
                    } else { // Nur KIs übrig oder nur der Host war drin
                    ...
                    // Sicherstellen, dass das richtige Objekt entfernt wird
                 ...
                 console.log("Spieler war bereits nicht im Spiel (leaveGame)."); // Kein Fehler, nur Info
            }
        } else {
             console.log("Spiel existierte nicht mehr beim Verlassen (leaveGame)."); // Kein Fehler
        }
    } catch (error) {
        ...
        // Wichtig: resetToLobby erst NACH den DB-Operationen
...
function handleGameUpdate(game) {
    ...
    // Toast-Nachrichten über Spieleränderungen (nur wenn im Spiel)
    if (game.status !== 'lobby') {
        try {
            ...
            if (currentPlayers.length > 0 && JSON.stringify(currentPlayers) !== JSON.stringify(lastKnownPlayers) && lastKnownPlayers.length > 0) {
                ...
                if (joined.length > 0 && !(joined.length === 1 && game.players.find(p=>p.name === joined[0])?.id === userId)) showToast(`${joined.join(', ')} ist beigetreten!`);
         ...
         sessionStorage.removeItem(`skyjoPlayers_${game.id}`); // Clear player list when back in lobby
    }
    ...
    // Standardmäßig alles ausblenden, dann das Nötige einblenden
    loaderEl?.classList.add('hidden');
    ...
    if (status === 'lobby') {
        lobbyScreenEl?.classList.remove('hidden');
        lobbyStartEl?.classList.add('hidden'); // Erstmal Wartebildschirm zeigen
        lobbyWaitEl?.classList.remove('hidden');
        leaveGameButtonEl?.classList.remove('hidden'); // Verlassen-Button in Lobby anzeigen
        ...
        // Einstellungen nur für Host sichtbar/änderbar
        const settingsDiv = document.getElementById('game-settings');
        if(settingsDiv) settingsDiv.style.display = 'block'; // Block immer anzeigen, aber Felder disablen
        ...
        infoButtonEl?.classList.remove('hidden'); // Info auch am Ende
        gameContainerEl?.classList.remove('hidden'); // Spielbrett sichtbar lassen
        leaveGameButtonEl?.classList.remove('hidden'); // Verlassen auch am Ende
        renderGame(game); // Rendert das finale Brett
        showEndRoundModal(game); // Zeigt das Modal
    } else {
...
function renderGame(game) {
    ...
    if (!me) {
        ...
        // Nicht resetten, handleGameUpdate prüft das
        return;
    }
    ...
    opponents.forEach(opp => {
        ...
        // KORREKTUR: Padding entfernt (p-0)
        oppContainer.className = 'flex flex-col items-center p-0 rounded-lg relative';
        const isCurrentPlayer = game.players[game.currentPlayerIndex]?.id === opp.id;
        if (isCurrentPlayer && game.status === 'playing') oppContainer.classList.add('bg-red-900/50', 'shadow-lg'); // Highlight nur im Spiel

        // KORREKTUR: Explizites gap-x-2 gap-y-2 und justify-center
        let gridHtml = `<div id="grid-${opp.id}" class="grid grid-cols-[repeat(4,60px)] gap-x-2 gap-y-2 justify-center mb-2">`;
        (opp.grid || []).forEach(card => {
            ...
            // Karten immer zeigen, wenn Runde/Spiel vorbei
            else if (card.faceUp || game.status === 'round-over' || game.status === 'game-over') gridHtml += `<div class="card face-up" data-value="${card.value}">${card.value ?? '?'}</div>`;
            else gridHtml += `<div class="card face-down">SKYJO</div>`;
        });
        gridHtml += '</div>';

        const totalScoreDisplay = (game.status === 'lobby' || typeof opp.totalScore !== 'number') ? '' : `| Gesamt: ${opp.totalScore}`;
        ...
        // KORREKTUR: Überschrift auf text-xl gesetzt und mb-2 hinzugefügt für Konsistenz
        oppContainer.innerHTML = `<h2 class="text-xl font-bold text-center text-red-400 flex items-center justify-center mb-2">${opp.name || 'Unbekannt'} ${aiLabel} ${thinkingSpinner}</h2>${gridHtml}<p class="text-center text-lg font-semibold">Punkte: <span>${currentScore}</span> <span class="text-slate-400">${totalScoreDisplay}</span></p>`;
    ...
    (me.grid || []).forEach((card, index) => {
        const cardEl = document.createElement('div');
         if (!card) {
            ...
            // Klickbar nur, wenn ich dran bin, Spiel läuft UND die Phase passt
            if (!card.discarded && amICurrent && game.status === 'playing' && validPhase) {
                 cardEl.addEventListener('click', () => onPlayerCardClick(index));
                 cardEl.style.cursor = 'pointer'; // Cursor explizit setzen
            } else if (!card.discarded) {
                 cardEl.style.cursor = 'default'; // Nicht klickbar
            } else {
                 cardEl.style.cursor = 'not-allowed'; // Verworfen
            }
        }
        playerGridEl?.appendChild(cardEl);
    });
    const myCurrentScore = (game.status === 'playing') ? calculateScore(me.grid, true) : (me.score ?? 0);
    const myTotalScoreDisplay = (game.status === 'lobby' || typeof me.totalScore !== 'number') ? '' : `| Gesamt: ${me.totalScore}`;
    if(playerScoreEl) playerScoreEl.textContent = `${myCurrentScore} ${myTotalScoreDisplay}`;
...
function renderDeck(game) {
    ...
    if (!deckEmpty && amICurrent && isPlaying && game.turnPhase === 'DRAW') {
    ...
    if (game.selectedCard && (game.turnPhase === 'DECK_DRAWN' || game.turnPhase === 'DISCARD_DRAWN')) {
        ...
        // Klickbar nur, wenn vom Deck gezogen, um Ablegen auszulösen
        if (amICurrent && isPlaying && game.turnPhase === 'DECK_DRAWN') {
    ...
    if (!discardPileEmpty) {
        const topDiscard = game.discardPile[game.discardPile.length - 1];
         if (typeof topDiscard !== 'undefined' && topDiscard !== null) {
            ...
            // Klickbar, wenn ich dran bin und ziehen darf (DRAW) ODER wenn ich vom Deck gezogen habe (DECK_DRAWN zum Ablegen)
            if (amICurrent && isPlaying && (game.turnPhase === 'DRAW' || game.turnPhase === 'DECK_DRAWN')) {
...
function updateHighlightsAndMessage(game) {
    ...
    if (currentPlayer.isAI && game.status === 'playing') { // Spinner nur während des Spiels
         const spinnerEl = document.getElementById(`spinner-${currentPlayer.id}`);
         if (spinnerEl) spinnerEl.classList.remove('hidden');
    }

    if (game.status !== 'playing') {
         // Spiel vorbei oder Lobby
         setMessage(game.status === 'lobby' ? "Warte auf Spielstart..." : "Spiel beendet.");
         return;
    }
...
function onDrawnCardClick() {
    // Nur ausführen, wenn man dran ist, Spiel läuft und eine Karte vom Deck gezogen wurde
     if (!actionInProgress && currentGameId && userId) {
         getDoc(doc(db, getGameCollectionPath(), currentGameId)).then(gameSnap => {
             if (gameSnap.exists()) {
                 const game = gameSnap.data();
                 if (game.status === 'playing' && game.players[game.currentPlayerIndex]?.id === userId && game.turnPhase === 'DECK_DRAWN') {
                    console.log("Gezogene Karte geklickt -> löst Ablegen aus.");
                    onDiscardPileClick();
                 } else {
                     console.log("Klick auf gezogene Karte ignoriert (falsche Phase/Spieler).")
                 }
             }
         }).catch(err => console.error("Fehler beim Prüfen des Spielstatus für DrawnCardClick:", err));
     } else {
         console.log("Klick auf gezogene Karte ignoriert (Aktion läuft / kein Spiel).")
     }
}
...
async function swapCard(game, playerId, index, newCardValue, updatedDeck, updatedDiscardPile) {
    ...
    try {
        ...
        // WICHTIG: Prüfe Rundenende mit dem *neuen* Grid-Zustand des Spielers
        const roundEndTriggered = triggerEndRoundCheck({ ...game, players: updatedPlayers }, player); // Verwende updatedPlayers
        ...
        actionInProgress = false; // Fehler -> Aktion freigeben
    }
}
...
function triggerEndRoundCheck(game, player) {
    // Spiel muss laufen UND Runde darf noch nicht ausgelöst sein
    if (!game || game.status !== 'playing' || game.lastRoundTriggered) return false;
    if (!player || !player.grid) return false;
    // Prüfe, ob ALLE nicht-verworfenen Karten aufgedeckt sind
    return player.grid.every(card => !card || card.discarded || card.faceUp);
}


async function endTurn(gameDocRef, game, roundEndTriggered) {
    // KI-Timeout NUR löschen, wenn er existiert
    if (aiTurnTimeoutId) {
        console.log("Clearing existing AI timeout in endTurn.");
        clearTimeout(aiTurnTimeoutId);
        aiTurnTimeoutId = null;
    }
    ...
    try {
        ...
        if (playerCount === 0) { console.warn("endTurn: Keine Spieler mehr."); actionInProgress = false; return; } // Wichtig: actionInProgress freigeben
        ...
        // Runde wurde *in diesem Zug* ausgelöst UND war vorher noch nicht ausgelöst
        if (roundEndTriggered && !lastRoundTriggered && currentPlayer) {
            ...
            // Aktualisiere lokalen Zustand für die nächste Prüfung
        ...
        // Runde war bereits ausgelöst, prüfe ob jetzt der Auslöser wieder dran ist
            ...
            actionInProgress = false; // Runde vorbei -> Aktion freigeben
            return; // Kein weiterer Zug, kein KI-Start
        }

        // Verstecke den Spinner des (potentiell) vorherigen Spielers (falls KI)
        ...
        if (nextPlayer.isAI && hostId === userId) {
            ...
            aiTurnTimeoutId = setTimeout(async () => {
                const isCurrentTimeout = aiTurnTimeoutId === currentTimeoutId; // Prüfen, ob dies noch der gültige Timeout ist
                aiTurnTimeoutId = null; // Timeout verbraucht oder wurde überschrieben
                 if (!isCurrentTimeout) {
                     console.log(`KI-Zug für ${nextPlayer.name} abgebrochen (neuer Timeout gestartet).`);
                ...
                try {
                    // Hole den absolut neuesten Spielstand direkt vor der KI-Aktion
                    ...
                    // Doppelte Sicherheitsprüfung: Ist die KI wirklich noch dran und läuft das Spiel?
                    if (freshGame.players && freshGame.players.length > freshGame.currentPlayerIndex &&
                        ...
                        await runAITurn(freshGame, nextPlayer); // runAITurn gibt actionInProgress am Ende frei
                    } else {
                        ...
                        actionInProgress = false; // Wichtig: Freigeben, wenn KI nicht handelt
                    }
                } catch (error) {
            ...
            const currentTimeoutId = aiTurnTimeoutId; // ID merken
        } else {
             actionInProgress = false; // Wenn kein KI-Zug -> Aktion direkt freigeben
        }
...
async function runAITurn(game, aiPlayer) {
     ...
     // actionInProgress wird VOR dem Aufruf gesetzt, hier nicht nochmal
     ...
     try {
         ...
         // Robuste Prüfung, ob KI dran ist
         const isAITurn = game.players.length > game.currentPlayerIndex &&
                          game.players[game.currentPlayerIndex]?.id === aiPlayer.id;
         if (!isAITurn) {
            console.log(`KI-Zug für ${aiPlayer.name} abgebrochen: Nicht (mehr) dran.`);
            // actionInProgress wird in endTurn freigegeben, hier nicht
            return;
         }
        ...
        // IMMER aktuellen Spielstand holen VOR der Aktion, um Race Conditions zu minimieren
        const currentSnap = await getDoc(gameDocRef);
        if (!currentSnap.exists()) throw new Error("Spiel verschwunden vor KI-Aktion.");
        let currentGameData = currentSnap.data();
        currentGameData.id = currentSnap.id;
        // Erneute Prüfung direkt vor der Aktion
        const isStillAITurn = currentGameData.players && currentGameData.players.length > currentGameData.currentPlayerIndex &&
                              currentGameData.players[currentGameData.currentPlayerIndex]?.id === aiPlayer.id &&
        ...
        if (takeDiscard) {
            soundManager.play('draw');
            const drawnCard = discardPileCopy.pop(); // Karte aus lokaler Kopie nehmen
            ...
            // swapCard holt sich intern den neuesten Stand nicht nochmal, arbeitet mit currentGameData
            await swapCard(currentGameData, aiPlayer.id, bestSwapDiscard.index, drawnCard, deckCopy, discardPileCopy);

        } else {
            soundManager.play('draw');
            if (deckCopy.length === 0) { // Deck mischen
            ...
            // Holen des aktuellsten Stands vor der Aktion (redundant, da oben schon, aber sicher)
            const snapBeforeAction = await getDoc(gameDocRef);
             if (!snapBeforeAction.exists()) throw new Error("Spiel verschwunden vor KI-Aktion 2.");
             currentGameData = snapBeforeAction.data();
             currentGameData.id = snapBeforeAction.id;
             const isStillAITurn2 = currentGameData.players && currentGameData.players.length > currentGameData.currentPlayerIndex &&
                                   currentGameData.players[currentGameData.currentPlayerIndex]?.id === aiPlayer.id &&
                                   currentGameData.status === 'playing';
             if (!isStillAITurn2) {
                 console.log(`KI-Zug für ${aiPlayer.name} abgebrochen: Zustand änderte sich vor Aktion 2.`);
                 return; // Beendet runAITurn
             }
            ...
            if (discardAndFlipMode) {
                ...
                discardPileCopy.push(drawnCard); // Zur lokalen Kopie hinzufügen
                ...
                } else { // Fallback, sollte nicht passieren, wenn hasFaceDown true war
        ...
        // Robustes Fehlerhandling: Versuche, den Zug trotzdem weiterzugeben
        try {
            // Hole den *letzten bekannten* gültigen Spielstand (könnte veraltet sein!)
            if (currentGameId && game && game.players[game.currentPlayerIndex]?.id === aiPlayer?.id) {
                 ...
                 // Hole den neuesten Stand, bevor endTurn aufgerufen wird
                 const currentSnap = await getDoc(currentDocRef);
                 if (currentSnap.exists()) {
                     const currentState = currentSnap.data();
                     currentState.id = currentSnap.id;
                     // Prüfe *erneut*, ob die KI noch dran ist, bevor der Zug weitergegeben wird
                     if (currentState.players && currentState.players.length > currentState.currentPlayerIndex && currentState.players[currentState.currentPlayerIndex]?.id === aiPlayer?.id) {
                        ...
                        await endTurn(currentDocRef, currentState, false); // Übergibt den neuesten Stand
                     } else {
                         console.log("Konnte Zug nach KI-Fehler nicht sicher weitergeben, Zustand hat sich geändert.");
                         actionInProgress = false; // Manuell freigeben
                     }
                 } else {
                      console.log("Konnte Zug nach KI-Fehler nicht weitergeben, Spiel existiert nicht mehr.");
                      actionInProgress = false; // Manuell freigeben
                 }
            } else {
                 actionInProgress = false; // Manuell freigeben, wenn Bedingungen nicht passen
            }
        } catch (endTurnError) {
         ...
         // Spinner wird in endTurn versteckt ODER wenn KI nicht mehr dran ist
         if (spinnerEl) spinnerEl.classList.add('hidden');
         // actionInProgress wird jetzt in endTurn freigegeben, oder im Fehlerfall oben
         console.log(`KI ${aiPlayer?.name} beendet runAITurn.`);
    }
}
...
function findBestAISwap(grid, newCardValue, difficulty) {
    ...
    for (let i = 0; i < grid.length; i++) {
        const card = grid[i];
         if (card && card.faceUp && !card.discarded) {
             const cardValue = typeof card.value === 'number' ? card.value : difficulty.ASSUMED_FACEDOWN_VALUE; // Verwende Annahme für faceUp, aber ungültigen Wert
    ...
    if (bestIdx === -1) return null; // Kein Tausch möglich/sinnvoll
    if (bestIdx < 0 || bestIdx >= grid.length) return null; // Ungültiger Index
...
function findWorstCardIndex(grid, newCardValue) {
     if (newCardValue === null || typeof newCardValue !== 'number' || !grid) {
          ...
          return firstValid === -1 ? 0 : firstValid; // Fallback auf Index 0 oder ersten gültigen
     }
    let worstIdx = -1; let minBenefit = Infinity;
    for (let i = 0; i < grid.length; i++) {
        const card = grid[i];
         // Finde die schlechteste *aufgedeckte* Karte zum Tauschen
         if (card && card.faceUp && !card.discarded) {
             const cardValue = typeof card.value === 'number' ? card.value : -Infinity; // Ungültige Werte sind sehr schlecht
            const benefit = cardValue - newCardValue;
            if (benefit < minBenefit) { minBenefit = benefit; worstIdx = i; }
        }
    }
     // Wenn keine aufgedeckte Karte gefunden wurde, nimm die erste nicht verworfene Karte
     const fallbackIndex = grid.findIndex(c => c && !c.discarded);
    const resultIndex = worstIdx === -1 ? fallbackIndex : worstIdx;
    return (resultIndex >= 0 && resultIndex < grid.length) ? resultIndex : -1; // Gib -1 zurück, wenn kein gültiger Index
}
...
async function startGame() {
    ...
    try {
        ...
        allPlayers.forEach(p => {
            ...
            flipRandomCards(p.grid, game.settings.flippedCards); p.score = 0; // Score zurücksetzen
        });
        const discardPile = (deck.length > 0) ? [deck.pop()] : [0]; // Sicherstellen, dass Ablagestapel nicht leer ist
        ...
        showToast(error.message || "Fehler beim Spielstart!", true);
         ...
         actionInProgress = false; // Aktion beendet
    }
}
...
function flipRandomCards(grid, count) {
    ...
    const shuffledIndices = shuffleDeck(faceDownIndices); // shuffleDeck gibt neues Array zurück
    const indicesToFlip = shuffledIndices.slice(0, count);
    indicesToFlip.forEach(idx => {
         if (grid[idx]) { // Sicherstellen, dass Index existiert
...
async function calculateFinalScores(gameDocRef, game) {
    ...
    try {
        ...
        updatedPlayers = updatedPlayers.filter(p => p); // Entferne null/undefined Spieler

        let roundScores = {}; // Um Original-Rundenscores vor Strafe zu speichern
        updatedPlayers.forEach(player => {
            if (!player || !player.grid) return;
            // Alle Karten aufdecken
            player.grid.forEach(card => { if(card) card.faceUp = true });
            // Spalten prüfen und ggf. als discarded markieren (wichtig VOR Score-Berechnung)
            ...
            roundScores[player.id] = player.score; // Original-Score speichern
        });
        ...
        // Finde den niedrigsten Original-Rundenscore
        ...
        // Strafe anwenden, falls nötig
        if (endingPlayerIndex !== -1) {
            const endingP = updatedPlayers[endingPlayerIndex];
            // Verwende Original-Score für die Bedingung
            const originalScore = roundScores[endingP.id];
            if (typeof originalScore === 'number' && originalScore > lowestRoundScore && originalScore > 0) {
                console.log(`Strafe für ${endingP.name}: ${originalScore} -> ${originalScore * 2}`);
                endingP.score = originalScore * 2; // Wende Strafe auf den Player-Objekt-Score an
            }
        }
        ...
        updatedPlayers.forEach(p => {
            if (!p) return;
            p.totalScore = (p.totalScore || 0) + (p.score || 0); // Addiere (ggf. bestraften) Score
            if (p.totalScore >= (game.settings.goalScore || 100)) gameOver = true;
        });

        // Scores für die Anzeige im Modal (enthält bestraften Score)
        const lastRoundScoresForDisplay = updatedPlayers.map(p => ({ id: p.id, name: p.name || '??', score: p.score ?? 0, originalScore: roundScores[p.id] ?? 0 })); // Original Score für Strafenanzeige speichern
        ...
        // Das Modal wird durch den Snapshot-Listener angezeigt
...
function showEndRoundModal(game) {
    ...
    // Sortiere nach dem Score in lastRoundScores für korrekte Anzeige
    const sortedPlayersForDisplay = [...validPlayers].sort((a, b) => {
         const scoreA = game.lastRoundScores?.find(s => s.id === a.id)?.score ?? Infinity;
         const scoreB = game.lastRoundScores?.find(s => s.id === b.id)?.score ?? Infinity;
         return scoreA - scoreB;
    });

    // Finde den niedrigsten *Original*-Score dieser Runde
     const originalScores = game.lastRoundScores?.map(s => s.originalScore).filter(s => typeof s === 'number') || [];
     const lowestOriginalScore = originalScores.length > 0 ? Math.min(...originalScores) : Infinity;
    ...
    sortedPlayersForDisplay.forEach(p => {
        ...
        // Finde den Score aus lastRoundScores für diesen Spieler
        const roundScoreData = game.lastRoundScores?.find(s => s.id === p.id);
        const displayScore = roundScoreData?.score ?? '?';
        const originalScore = roundScoreData?.originalScore ?? null;

        let penaltyIndicator = '';
        // Strafe anzeigen, wenn ID übereinstimmt UND Original-Score die Bedingungen erfüllt
        if (p.id === game.playerWhoEndedRoundId && originalScore !== null && typeof originalScore === 'number' && originalScore > lowestOriginalScore && originalScore > 0) {
            penaltyIndicator = `<span class="text-red-400 font-bold ml-2">(x2 Strafe!)</span>`;
        }


        scoreDiv.className = `p-3 rounded-lg ${colorClass}`;
        scoreDiv.innerHTML = `<p class="font-semibold text-lg ${textColorClass}">${nameLabel}</p><p>Runde: <span class="font-bold text-xl">${displayScore}</span>${penaltyIndicator}</p><p>Gesamt: <span class="font-bold text-xl">${p.totalScore ?? '?'}</span></p>`;
    ...
    if (status === 'game-over') {
        ...
        if (winner) {
            ...
            // Farbe basierend darauf, ob ICH der Gewinner bin oder ein Gegner/KI
            const winnerColor = winners.length > 1 ? 'text-yellow-400' : (winners[0].id === userId ? 'text-blue-400' : 'text-red-400');
        ...
        const winner = sortedPlayersForDisplay[0]; // Bereits nach Rundenscore sortiert
         if (winner) {
             // Finde den Score des Gewinners aus lastRoundScores
             const winnerScore = game.lastRoundScores?.find(s => s.id === winner.id)?.score ?? Infinity;
             const winners = sortedPlayersForDisplay.filter(p => (game.lastRoundScores?.find(s => s.id === p.id)?.score ?? Infinity) === winnerScore);
...
async function startNextRound() {
    ...
    try {
        ...
        if (game.status === 'game-over') {
            ...
            const humanPlayers = game.players.filter(p => p && !p.isAI);
            // Setze Scores und Grids für *alle* menschlichen Spieler zurück
            humanPlayers.forEach(p => { if(p) { p.grid = []; p.score = 0; p.totalScore = 0; } });

            await updateDoc(gameDocRef, {
                status: 'lobby',
                // Behalte nur die menschlichen Spieler mit zurückgesetzten Scores
                players: humanPlayers.filter(p => p),
                lastRoundScores: [], // Leere Scores für neue Lobby
                // Reset game state fields
                lastRoundTriggered: false, playerWhoEndedRoundId: null,
                deck: [], discardPile: [], currentPlayerIndex: 0, turnPhase: 'DRAW', selectedCard: null
            });
            // Snapshot-Listener wird die UI aktualisieren
            ...
            // Wichtig: Kopie erstellen, um Seiteneffekte zu vermeiden
            let updatedPlayers = JSON.parse(JSON.stringify(game.players));
            updatedPlayers = updatedPlayers.filter(p => p); // Filtere null/undefined

            // Finde den Gewinner-Index aus der *letzten* Runde
            ...
            // Fallback, wenn Gewinner nicht gefunden oder keine Scores vorhanden
            if (startingPlayerIndex === -1) {
                // Wer war zuletzt dran? Der Nächste startet.
                const lastPlayerIndex = game.currentPlayerIndex;
                startingPlayerIndex = updatedPlayers.length > 0 ? (lastPlayerIndex + 1) % updatedPlayers.length : 0;
                console.log("Kein Rundengewinner gefunden/Scores leer, nächster Spieler startet:", startingPlayerIndex);
            } else {
                 console.log("Rundengewinner startet:", startingPlayerIndex);
            }


            updatedPlayers.forEach(p => {
                if (!p) return;
                p.grid = []; // Leeres Grid für neue Runde
                for (let i = 0; i < 12 && deck.length > 0; i++) p.grid.push({ value: deck.pop(), faceUp: false, discarded: false });
                flipRandomCards(p.grid, game.settings.flippedCards);
                p.score = 0; // Nur Runden-Score zurücksetzen
            });

            const discardPile = (deck.length > 0) ? [deck.pop()] : [0]; // Neue Startkarte
            ...
            await updateDoc(gameDocRef, {
                ...
                turnPhase: 'DRAW',
                lastRoundTriggered: false, // Zurücksetzen für neue Runde
                playerWhoEndedRoundId: null, // Zurücksetzen
                selectedCard: null // Zurücksetzen
                // lastRoundScores bleiben erhalten für die Anzeige
            });
            // Snapshot-Listener wird die UI aktualisieren
        }
    } catch (error) {
        ...
        if (game && game.status !== 'game-over' && loaderEl) loaderEl.classList.add('hidden'); // Loader nur bei Rundenstart-Fehler ausblenden
        resetToLobby(true); // Sicherer Fallback zur Lobby bei Fehlern
    } finally {
         if(nextRoundButtonEl) nextRoundButtonEl.disabled = false; // Button wieder aktivieren
         actionInProgress = false; // Aktion beendet
    }
}
